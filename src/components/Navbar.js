...
export default function Navbar({ content = {} }) {
  ...
  return (
    <>
      ...
      {/* Mobile menu — separate from nav so it isn't affected by nav transform */}
      {menuOpen && (
        <div className="fixed inset-0 z-[55] bg-foreground md:hidden">
          <div className="flex items-center justify-between px-5 py-4 sm:px-6 sm:py-5">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="relative flex h-10 w-10 items-center justify-center rounded-full text-white/70 hover:text-white"
                aria-label="Open cart"
              >
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
              ...
              <button
                onClick={() => setMenuOpen(false)}
                className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-white/30 text-white hover:border-white focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          ...
          <div className="flex flex-col items-center gap-10 pt-20">
            {ALL_LINKS.map((link) => (
              <a
                ...
                href={link.href}
                onClick={() => setMenuOpen(false)}
                className="font-display text-4xl tracking-[0.12em] text-white"
              >
                {link.label}
              </a>
            ))}
